...
const tipoVigaNombre = (tipo) => {
  switch (tipo) {
    case 1:
      return "Viga corona delgada";
    case 2:
      return "Viga corona gruesa";
    case 3:
      return "Viga normal";
    case 4:
      return "Viga carga lateral";
    case 5:
      return "Viga carga central";
    default:
      return "Viga";
  }
};
...
export default function CotizacionVigasPDF({ cotizacion, params }) {
  ...
  const vigas = Array.isArray(params?.vigas) ? params.vigas : [];
  const vigasCimiento = Array.isArray(params?.vigas_cimiento) ? params.vigas_cimiento : [];
  const cintas = Array.isArray(params?.cintas) ? params.cintas : [];

  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <Text style={styles.title}>COTIZACIÓN DE VIGAS / CINTAS</Text>
          <Text style={styles.subtitle}>Sistema de Presupuesto de Construcción</Text>
          <Text style={styles.date}>Fecha: {fechaActual}</Text>
        </View>

        <View style={styles.card}>
          <Text style={styles.cardTitle}>Resumen de elementos</Text>
          <Text style={styles.cardText}>Vigas estructurales: {vigas.length}</Text>
          <Text style={styles.cardText}>Vigas de cimiento: {vigasCimiento.length}</Text>
          <Text style={styles.cardText}>Cintas: {cintas.length}</Text>
        </View>

        {vigas.length > 0 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Detalle de vigas</Text>
            <View style={styles.list}>
              {vigas.map((viga, index) => (
                <Text key={`viga-${index}`} style={styles.listItem}>
                  #{index + 1}: {viga.largo} cm · {tipoVigaNombre(viga.tipo)} ·{" "}
                  {tipoLadrilloNombre(viga.id_ladrillo)} · Cantidad: {viga.cantidad}
                </Text>
              ))}
            </View>
          </View>
        ...
        {vigasCimiento.length > 0 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Detalle de vigas de cimiento</Text>
            <View style={styles.list}>
              {vigasCimiento.map((viga, index) => (
                <Text key={`vc-${index}`} style={styles.listItem}>
                  #{index + 1}: {viga.largo} cm · Cantidad: {viga.cantidad}
                </Text>
              ))}
            </View>
          </View>
        ...
        {cintas.length > 0 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Detalle de cintas</Text>
            <View style={styles.list}>
              {cintas.map((cinta, index) => (
                <Text key={`cinta-${index}`} style={styles.listItem}>
                  #{index + 1}: {cinta.largo} cm · {tipoLadrilloNombre(cinta.id_ladrillo)} ·
